...
Vector_name = 'MobilenetV2'
# Vector_name = 'Inception'
# Vector_name = 'ResNet152'
...
if Vector_name == 'MobilenetV2':
    dst_name = 'UCF101_MobilenetV2_64_frame'
    base_model = tf.keras.applications.MobileNetV2(include_top=False, weights='imagenet')
    
if Vector_name == 'Inception':
    dst_name = 'UCF101_Inception_64_frame'
    base_model = tf.keras.applications.InceptionV3(include_top=False, weights='imagenet')

if Vector_name == 'ResNet152':
    dst_name = 'UCF101_Resnet152_64_frame'
...
for root, dirs, files in os.walk(src_folder):
    ...
    for file in files:
        ...
        with open(os.path.join(root, file), "r") as auto:
            ...
            if Vector_name == 'MobilenetV2' or Vector_name == 'Inception':
                batch_features = feature_extraction_model(datasets)
                batch_features = np.array(batch_features)

            if Vector_name == 'ResNet152':
                # print(datasets.shape)
                # Due to the memory shortage, resnet output divided two parts then appended.
                batch_features_1 = feature_extraction_model(datasets[0:int(n_feature_frame/2)])
                batch_features_1 = np.array(batch_features_1)

                batch_features_2 = feature_extraction_model(datasets[int(n_feature_frame/2):n_feature_frame])
                batch_features_2 = np.array(batch_features_2)

                batch_features = np.append(batch_features_1, batch_features_2, axis=0)
