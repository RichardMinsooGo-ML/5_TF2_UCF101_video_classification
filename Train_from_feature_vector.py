...
# 8 * 8 * feature_vec_size to 1 x feature_vec_size, as we don't need spatial information
...
for img, batch_paths in tqdm.tqdm(dataset):
    ...
    for features, path in zip(batch_features.numpy(), batch_paths.numpy()):
        if path != current_path and current_path is not None:
            ...
            output_path = current_path.decode().replace('UCF101_asource', 'UCF101_Inception_64_frame')
            # print(output_path)
...
def make_generator(file_list):
    def generator():
        ...
        for path in file_list:
            ...
            padded_sequence = np.zeros((SEQUENCE_LENGTH, feature_vec_size))
...
train_dataset = tf.data.Dataset.from_generator(make_generator(train_list),
                 ...
                 output_shapes=((SEQUENCE_LENGTH, feature_vec_size), (len(LABELS))))
